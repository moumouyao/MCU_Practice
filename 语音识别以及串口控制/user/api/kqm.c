...
void air_analysis(void){
		if(usart_get_kqm__RXflag() == 1){
			printf("VOC = %.2fPPM\t", ((usart_kqm_RXpacket[1] << 8) | usart_kqm_RXpacket[2]) * 0.1);
			printf("CHO = %.2fmg/m3\t", ((usart_kqm_RXpacket[3] << 8) | usart_kqm_RXpacket[4]) * 0.01);
			printf("CO2 = %dPPM\r\n", ((usart_kqm_RXpacket[5] << 8) | usart_kqm_RXpacket[6]));
