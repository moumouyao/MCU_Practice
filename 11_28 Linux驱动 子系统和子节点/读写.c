...
int myopen(struct inode *inode, struct file *file){
	int i;
	file->private_data = (void *)(&inode->i_rdev);
	for(i = 0; i < 4; i++){
...
ssize_t myread (struct file *file, char __user *buf, size_t size, loff_t * loff){
	...
	unsigned long ret;
	for(i = 0; i < 4; i++){
		if((*(dev_t *)(file->private_data)) == mydev[i]){
			printk("进入read函数\n");
			ret=copy_to_user(buf, "hello",6);
			break;
		}
...
ssize_t mywrite (struct file *file, const char __user *buf, size_t size, loff_t *loff){
	int i;
	char str[8];
	unsigned long ret;
	for(i = 0; i < 4; i++){
		if((*(dev_t *)(file->private_data)) == mydev[i]){
			printk("进入write函数\n");
			ret=copy_from_user(str, buf,strlen(buf));
			printk("%s\n", str);
...
static int __init myled_init(void){
	...
	for(i = 0; i < 4; i++){
		...
		myfops[i].read = myread;
		myfops[i].write = mywrite;
